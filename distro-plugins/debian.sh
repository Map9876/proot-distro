...
TARBALL_URL['aarch64']="https://map987.us.kg/https://github.com/termux/proot-distro/releases/download/v4.7.0/debian-bookworm-aarch64-pd-v4.7.0.tar.xz" #用代理下载，通过运作proot-distro install debian输出这个连接，搜源代码找到这里，问题是apt install proot-distro 和从github下载文件后运行sh文件，都可以使用proot-distro命令，是通过修改bashrc吗 
